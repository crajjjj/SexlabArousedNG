...
Do **not** check separately for `OSLAroused.esp` — OSL ships its own `SexLabAroused.esm`, so `GetModByName("SexLabAroused.esm")` will be present on both installs.
...
Function ModifyArousal(Actor who, Float value) Global
    ...
    if SupportsSLANG()
        ; Real SLA NG: use the ModEvent API (see Dynamic Effects section above).
        int handle = ModEvent.Create("slaModArousalEffect")
        if handle
            ModEvent.PushForm(handle, who)
